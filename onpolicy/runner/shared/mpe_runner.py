...
class MPERunner(Runner):
    ...
    def eval(self, total_num_steps):
        num_episodes = self.all_args.eval_episodes

        # Collect one summed-reward array per episode per thread.
        # Each entry shape: [n_eval_threads, num_agents, 1]
        all_ep_rewards = []
        ...
        for _ in range(num_episodes):
            ...
            for eval_step in range(self.episode_length):
                ...
                ep_step_rewards.append(eval_rewards)

                eval_rnn_states[eval_dones == True] = np.zeros(((eval_dones == True).sum(), self.recurrent_N, self.hidden_size), dtype=np.float32)
                eval_masks = np.ones((self.n_eval_rollout_threads, self.num_agents, 1), dtype=np.float32)
                eval_masks[eval_dones == True] = np.zeros(((eval_dones == True).sum(), 1), dtype=np.float32)

            # Sum over steps → [n_eval_threads, num_agents, 1]
            all_ep_rewards.append(np.sum(np.array(ep_step_rewards), axis=0))

        # Stack episodes then flatten episodes × threads into one sample axis.
        # Shape: [num_episodes * n_eval_threads, num_agents, 1]
        ep_rewards_sum = np.stack(all_ep_rewards, axis=0).reshape(-1, self.num_agents, 1)
        total_eval_episodes = num_episodes * self.n_eval_rollout_threads
        ...
        print("eval average episode rewards of agent: "
              ...
              + " (over %d episodes)" % total_eval_episodes)
